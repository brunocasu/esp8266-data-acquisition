...
#define APP_VERSION "app-ftp-ap-sht30-hp3038_v1.1"
...
void dataAcquisition() {
  double hp303b_temp;
  int32_t temp; // not used
  int32_t hp303b_pres;
  int16_t hp303b_oversampling = 2; // Highest precision
  
  sht30_1_handler.cTemp = 0;
  sht30_2_handler.cTemp = 0;
  if (LittleFS.exists(hp303b_file_path)){ // File exists
    hp303b_handler.begin();
    if(hp303b_handler.measureTempOnce(temp, hp303b_oversampling) == 0){ // Temperature reading OK
      if(hp303b_handler.measurePressureOnce(hp303b_pres, hp303b_oversampling) == 0){ // Pressure reading OK
        hp303b_temp = hp303b_handler.returnDoublePrecisionTemp(); // Adapted function to retrieve decimal precision temperature (LOLIN_HP303B driver returns int32_t)
